...
| 項目 | 内容 |
|---|---|
| 入力 | 40×50 グリッド上の疎な RSS 観測（100〜1200点／全2000セル、つまり **5〜60% しか観測できない**） |
...
**② 残差学習（学習しやすさの向上）**
生の RSS をそのまま入力せず、自由空間伝搬損失（FSPL）の理論値との**差分**を学習対象にします。
...
探索する設計**（学習率2通り × 損失重み9通り × 3条件）に組み直しました。
...
### 長時間実験の運用性・再現性
70本規模の学習を数十時間かけて回す必要があったため、運用面も設計しました。
